...
function applyFocusFilter() {
  ...
  CY.edges().forEach(e => {
    if (!keep.has(e.data("source")) || !keep.has(e.data("target"))) {
      e.addClass("faded");
    }
  });
  // Re-layout visible subset with dagre so the focus subgraph is tightly
  // arranged (instead of scattered across the original 82-region coordinates).
...
function showNodeDetail(id) {
  ...
  //
  // "Shipped" — for the purpose of deciding whether this upstream still
  // blocks its downstream. Per the README's closure semantics:
  //   - public  = release / changelog announced (released to the world)
  //   - bridged = upstream/downstream issues closed + smoke passed (integration verified)
  //
  // bridged is the level where downstream can RELY on this region. A
  // public region with open bugs is announced but its integration smoke
  // hasn't passed — so it still blocks downstream from a clean integration.
  //
  // Logic:
  //   1) closure >= bridged → shipped (integration verified)
  //   2) closure == public AND zero open issues → shipped (released cleanly)
  //   3) milestone_ref + every tracked issue closed → shipped (pragmatic
  //      auto-detect for when closure stage hasn't been manually bumped)
  // Everything else: still blocking.
  const CLOSURE_RANK = {seed:0, obligation:1, scoped:2, public:3, bridged:4, mature:5};
  const isShipped = (k) => {
    const rr = REGIONS_DOC.regions[k];
    if (!rr) return false;
    const rank = CLOSURE_RANK[rr.closure] ?? 0;
    if (rank >= CLOSURE_RANK.bridged) return true;
    if (rank >= CLOSURE_RANK.public && (rr.issue_count || 0) === 0) return true;
    if (rr.milestone_ref) {
...
function renderAll() {
  ...
  if (CY && CY.nodes(":selected").length) {
    ...
    renderDefaultPanel();
    // On initial load (and when language toggles), apply the focus filter so
    // the graph echoes the side panel — only "currently attacking" subgraph
    // stays prominent; the rest fades.
    applyFocusFilter();
  }
}
